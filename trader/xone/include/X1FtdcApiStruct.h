...
/**
 * <table border="3">
 *      <tr> <th>名称</th>                        <th>类型</th> <th>描述</th>
 * <th>是否必填</th>               <th>取值范围说明</th></tr> <tr>
 * <td>AccountID</td>                   <td>TX1FtdcAccountIDType</td>
 * <td>资金账户ID</td>                                <td>M</td> <td></td> </tr>
 *      <tr> <td>LocalOrderID</td> <td>TX1FtdcLocalOrderIDType</td>
 * <td>本地委托号</td>                                <td>M</td>
 * <td>由API使用者维护，在同一个会话中不能重复，并且必须比上一次大</td></tr>
 *      <tr> <td>InstrumentID</td> <td>TX1FtdcInstrumentIDType</td>
 * <td>合约代码</td>                                  <td>M</td>
 * <td>支持目前国内4个期货交易所的所有合约，包括大商所/郑商所的套利合约</td>
 * </tr> <tr> <td>InsertPrice</td>                 <td>TX1FtdcPriceType</td>
 * <td>报单价格</td>                                  <td>M</td>
 * <td>当报单类型为市价时，该字段不起作用</td></tr> <tr> <td>OrderAmount</td>
 * <td>TX1FtdcAmountType</td>        <td>报单数量</td> <td>M</td>
 * <td>必须大于0的整数</td></tr> <tr> <td>TX1FtdcBuySellTypeType</td>
 * <td>BuySellType</td>              <td>买卖标志</td> <td>M</td>
 * <td>1买；2卖</td></tr> <tr> <td>TX1FtdcOpenCloseTypeType</td>
 * <td>OpenCloseType</td>            <td>开平标志</td> <td>M</td>
 * <td>1开仓；2平仓；4平今</td></tr> <tr> <td>TX1FtdcSpeculatorType</td>
 * <td>Speculator</td>               <td>投保类型</td> <td>M</td>
 * <td>支持投机、套利、套保</td></tr> <tr> <td>TX1FtdcInsertType</td>
 * <td>InsertType</td>               <td>自动单类别</td> <td>M</td>
 * <td>(默认为普通订单)(此版本不用)</td></tr> <tr> <td>TX1FtdcOrderTypeType</td>
 * <td>OrderType</td>                <td>报单类型</td> <td>M</td> <td>支持限价
 * 、市价；上期所合约不支持市价，均按限价进行处理</td></tr> <tr>
 * <td>TX1FtdcOrderPropertyType</td>    <td>OrderProperty</td>
 * <td>报单附加属性</td>                              <td>M</td>
 * <td>支持None、FAK、FOK</td></tr> <tr> <td>TX1FtdcInstrumentTypeType</td>
 * <td>InstrumentType</td>           <td>合约类型</td> <td>M</td>
 * <td>可选值：期货、期权</td></tr> <tr> <td>TX1FtdcAmountType</td>
 * <td>MinMatchAmount</td>           <td>最小成交量</td> <td>M</td>
 * <td></td></tr> <tr> <td>TX1FtdcReservedType</td> <td>ReservedType2</td>
 * <td>预留字段2</td>                                 <td>NM</td> <td></td></tr>
 *      <tr> <td>TX1FtdcCustomCategoryType</td>   <td>CustomCategory</td> <td
 * bgcolor="gray">自定义类别(此版本不用)</td>     <td>BLK</td>
 * <td>自定义类别(此版本不用)</td></tr> <tr> <td>TX1FtdcPriceType</td>
 * <td>ProfitLossPrice</td>          <td
 * bgcolor="gray">止盈止损价格(此版本不用)</td>   <td>NM</td>
 * <td>止盈止损价格(此版本不用)</td></tr>
 * </table>
 */
///请求报单数据类型(基本报单)
...
/**
 * <table border="3">
 *      <tr> <th>名称</th>         <th>类型</th> <th>描述</th> <th>是否必填</th>
 * <th>取值范围说明</th></tr> <tr> <td>AccountID</td>
 * <td>TX1FtdcAccountIDType</td>     <td bgcolor="gray">资金账户ID
 * (此版本不用)</td> <td>NM</td>         <td></td> </tr> <tr> <td>X1OrderID</td>
 * <td>TX1FtdcX1OrderIDType</td>     <td>柜台委托号</td> <td>NM</td> <td>1.
 * X1OrderID和LocalOrderID任意输入其中一个值即可<br>2.
 * 如果X1OrderID和LocalOrderID两个值都输入了，程序将优先使用X1OrderID（两者同时小于等于0时，则无法撤单成功）<br>3.
 * 当柜台重启后，需要使用X1OrderID进行撤单</td> </tr> <tr> <td>LocalOrderID</td>
 * <td>TX1FtdcLocalOrderIDType</td>  <td>本地委托号</td> <td>NM</td>
 * <td>当客户端使用API登出，再登陆到前置后，此时需要用LocalOrderID 加
 * SessionID进行撤单</td></tr> <tr> <td>InstrumentID</td>
 * <td>TX1FtdcInstrumentIDType</td>  <td bgcolor="red">合约代码(此版本不用)</td>
 * <td>NM</td>         <td></td></tr> <tr> <td>RequestID</td>
 * <td>TX1FtdcRequestIDType</td>     <td>请求ID</td> <td>NM</td> <td></td></tr>
 *      <tr> <td>SessionID</td>    <td>TX1FtdcSessionIDType</td> <td>会话ID</td>
 * <td>NM</td>         <td></td></tr>
 * </table>
 */
///撤单数据类型
...
struct APISTRUCT CX1FtdcRspPriOrderField {
  ...
  TX1FtdcOrderAnswerStatusType
      OrderStatus;  // 委托状态  在FAK
                    // FOK情况返回交易所已经接受，但尚未成交8，失败返回7，其他情况一律返回未成交3
  TX1FtdcSessionIDType SessionID;            // 会话ID
 ...
 * @brief 委托查询数据结构
 * @details
 * 这里有丰富的查询条件设置，如果想查询所有委托记录，则只需要输入AccountID和InstrumentType即可。
 ...
 * @brief 成交查询数据结构
 * @details
 * 这里有丰富的查询条件设置，如果想查询所有委托记录，则只需要输入AccountID和InstrumentType即可。（注意OrderType查询条件该版本暂不支持）
 */
...
struct APISTRUCT CX1FtdcRspOrderField {
  ...
  TX1FtdcLocalOrderIDType
      LocalOrderID;  // 本地委托号
                     // 是下单时该单子的本地委托号，如果同一账号从多个客户端下单，则查询返回的LocalOrderID可能是重复的
...
struct APISTRUCT CX1FtdcQuoteRtnField {
  ...
  TX1FtdcX1OrderIDType
      X1OrderID;  // 柜台委托号
                  // 返回的柜台委托号为第一边的柜台委托号，第二边的柜台委托号实际上是第一遍柜台委托号加1，不需要用户关心。后续撤单操作指定这个柜台委托号来进行双边撤单，不支持只撤销其中一边的操作
  TX1FtdcAccountIDType AccountID;            // 资金账号(此版本不适用)
  TX1FtdcInstrumentTypeType InstrumentType;  // 合约类型
...
struct APISTRUCT CX1FtdcQuoteMatchRtnField {
  ...
  TX1FtdcLargeMarginDirectType
      LargeMarginDirect;       // 大边保证金方向(此版本不适用)
  TX1FtdcQuoteIDType QuoteID;  // 询价编号
  TX1FtdcCustomCategoryType CustomCategory;  // 自定义类别
  TX1FtdcLocalOrderIDType PriFlowNo;         // 私有流编号
};
...
/**
 * @details
 * 当exchangeID为空时，表示查询各交易所的所有合约代码，也可指定查询某交易所的所有合约代码信息。
 */
