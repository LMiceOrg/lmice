 ...
 * @brief 用户响应接口
 * @details
 * 实现了事件通知接口，用户需要实现此类接口，编写事件处理方法来处理用户感兴趣的事件。
 * @see Init
 */
 ...
 * @brief 用户请求接口
 * @details
 * 实现了用户请求接口，用户实例化对象后，可以直接调用接口进行登录、报单、撤单、查询等操作
 * @warning
 *         <ul>
 *           <li>
 * 用户调用接口前，需要进行memset清空结构体内容，再填写需要的字段信息 <li>
 * 接口线程安全 <li>
 * 买卖，开平、投保、订单类型、订单属性这些字段因为使用了位域处理，在API端不做数值有效性检查判断，按照头文件指定数值输入，输入其他值也会被转换为不确定数值
