...
CandleStickSetting::CandleStickSetting(QWidget *parent)
    : QWidget(parent), ui(new Ui::CandleStickSetting) {
    ui->setupUi(this);
    ...
    connect(m_filter, SIGNAL(loadFileEvent(QString, int, int)), this,
            SLOT(getOpenFile(QString, int, int)));
    connect(m_filter, SIGNAL(updateSettingEvent()), this,
            SLOT(updateSetting()));

    m_helper = new PythonHelper(this, this);
    CandleStickSetting::callFunction func =
        static_cast<WindowCallable::callFunction>(&CandleStickSetting::call);
...
int CandleStickSetting::call(qlonglong wid, const char *method,
                             QVariant *params, size_t size) {
...
void CandleStickSetting::getOpenFile(const QString &type, int begin, int end) {
    QString name =
        QFileDialog::getOpenFileName(this, tr("Load %1 data").arg(type));
...
void CandleStickSetting::updateSetting() {
    ...
    if (code.isEmpty()) code = ui->textEdit->toPlainText();
...
EditPressFilter::EditPressFilter(QTextEdit *parent) { m_edit = parent; }
    ...
    if (event->type() == QEvent::ToolTip) {
        ...
        if (!cursor.selectedText().isEmpty()) {
            QToolTip::showText(helpEvent->globalPos(),
                               cursor.blockCharFormat().toolTip());
            qDebug() << cursor.selectedText()
                     << cursor.blockCharFormat().toolTip()
                     << cursor.blockCharFormat().foreground().color();
        } else
            QToolTip::hideText();
        return true;
        ...
        if (evt->key() == Qt::Key_Tab) {
            ...
            QString name = QFileDialog::getOpenFileName(
                NULL, tr("Load setting from file"));
            if (!name.isEmpty()) {
                QFile file(name);
        ...
        if (act == "load") {
            ...
            if (loadIndex < 0) goto continueProcess;

            QRegularExpression loadEndExp = QRegularExpression("\\)");
            QRegularExpressionMatch match = loadEndExp.match(text, loadIndex);
            int endIndex = match.capturedStart();
            if (endIndex < 0) goto continueProcess;

            int begin =
                block.position() + loadIndex + matchStart.capturedLength();
