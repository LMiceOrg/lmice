...
static inline long f_mktime(struct tm *pt) {
  ...
  return (pt->tm_year - 70) * tyear + (pt->tm_yday) * tday +
         (pt->tm_hour) * thour + pt->tm_min * tmin + pt->tm_sec;
}
...
static inline void readall_guava2(LS_ChinaL1Source *self) {
  ...
  /** Init */
  self->m_fsize = 0;
  if (self->m_list) free(self->m_list);
  ...
  do {
    ...
    /**! filter */
    if (LS_InstrumentFilter_check(self->m_ifilter, data->quot.InstrumentID) !=
        0)
      continue;
    ...
    if (((*(uint64_t *)&data->quot.AskPrice1) != dmax) &&
      ...
      *((uint64_t *)msg_dt->m_inst) = *((uint64_t *)(data->quot.InstrumentID));
      ...
      micro_time = (int64_t)data_time1 * 1000000LL +
...
static PyMemberDef ChinaL1Source_members[] = {
    ...
    ChinaL1Source_Prop_Def(m_time, T_LONG, "micro time") ChinaL1Source_Prop_Def(
        m_bid, T_DOUBLE, "bid price") ChinaL1Source_Prop_Def(m_offer, T_DOUBLE,
                                                             "bid price")
                ...
                ChinaL1Source_Prop_Def(m_volume, T_INT, "volume")
                    ChinaL1Source_Prop_Def(m_notional, T_DOUBLE, "notional")
                        ChinaL1Source_Prop_Def(m_limit_up, T_DOUBLE, "limit up")
                            ChinaL1Source_Prop_Def(m_limit_down, T_DOUBLE,
                                                   "limit down")

                                {"msg_size", T_INT,
                                 (Py_ssize_t)offsetof(LS_ChinaL1Source,
                                                      m_fsize),
                                 0, "size of messages"},
    {"msg_pos", T_INT, (Py_ssize_t)offsetof(LS_ChinaL1Source, m_fpos), 0,
     "message position"},
    {"inst_filter", T_OBJECT_EX,
     (Py_ssize_t)offsetof(LS_ChinaL1Source, m_ifilter), 0, "instrument filter"},
    //{"data_path", T_STRING, (Py_ssize_t)offsetof(LS_ChinaL1Source, m_path), 0,
    //"source path"},
    {NULL, 0, 0, 0, NULL} /* Sentinel */
};

static PyObject *ChinaL1Source_get_data_path(LS_ChinaL1Source *self,
                                             void *closure) {
  PyObject *obj;
  (void)closure;
...
static int ChinaL1Source_set_data_path(LS_ChinaL1Source *self, PyObject *value,
                                       void *closure) {
  char *v;
  (void)closure;
  if (value == NULL) {
  ...
  memcpy(self->m_path, v,
         strlen(v) > sizeof(self->m_path) - 1 ? strlen(v)
                                              : sizeof(self->m_path) - 1);

  return 0;
}

static PyObject *ChinaL1Source_get_m_inst(LS_ChinaL1Source *self,
                                          void *closure) {
  PyObject *obj;
  (void)closure;
...
static int ChinaL1Source_set_m_inst(LS_ChinaL1Source *self, PyObject *value,
                                    void *closure) {
  if (value == NULL) {
  ...
  memcpy(self->m_data.m_inst, v,
         strlen(v) > sizeof(self->m_data.m_inst) - 1
             ? strlen(v)
             : sizeof(self->m_data.m_inst) - 1);

  return 0;
}

static PyGetSetDef ChinaL1Source_getseters[] = {
    {"m_inst", (getter)ChinaL1Source_get_m_inst,
     (setter)ChinaL1Source_set_m_inst, "instrument", NULL},
    {"data_path", (getter)ChinaL1Source_get_data_path,
     (setter)ChinaL1Source_set_data_path, "data path", NULL},

    {NULL, NULL, NULL, NULL, NULL} /* Sentinel */
};

static int ChinaL1Source_clear(LS_ChinaL1Source *self) {
...
static void ChinaL1Source_dealloc(LS_ChinaL1Source *self) {
...
static PyObject *ChinaL1Source_repr(LS_ChinaL1Source *self) {
  return PyUnicode_FromFormat(
      ...
      "\t\"size\":%d\n"
      "}\n",
      (int64_t)self, self->m_path, self->m_data.m_inst, self->m_fsize);
}

static PyObject *ChinaL1Source_new(PyTypeObject *type, PyObject *args,
                                   PyObject *kwds) {
  ...
  if (self != NULL) {
    ...
    self->m_ifilter = (LS_InstrumentFilter *)LS_InstrumentFilter_Type.tp_new(
        &LS_InstrumentFilter_Type, 0, 0);
...
static int ChinaL1Source_init(LS_ChinaL1Source *self, PyObject *args,
                              PyObject *kwds) {
  ...
  if (name) return load_file(self, name, len);

  return 0;
}
static int ChinaL1Source_traverse(LS_ChinaL1Source *self, visitproc visit,
                                  void *arg) {
  (void)self;
  (void)visit;
  (void)arg;
...
static PyObject *ChinaL1Source_load(LS_ChinaL1Source *self, PyObject *args) {
  const char *name = NULL;
  int ret;
  int len;

  ret = PyArg_ParseTuple(args, "s#", &name, &len);
  if (!ret) return Py_False;
...
static PyObject *ChinaL1Source_at(LS_ChinaL1Source *self, PyObject *args) {
  int pos = 0;
  int ret;

  if (self->m_fsize <= 0) return Py_BuildValue("i", pos);

  ret = PyArg_ParseTuple(args, "i", &pos);
  if (!ret) return Py_BuildValue("i", pos);
...
static PyObject *ChinaL1Source_next(LS_ChinaL1Source *self) {
  if (self->m_fsize == 0) return NULL;

  ++self->m_fpos;
  if (self->m_fpos >= self->m_fsize) self->m_fpos = self->m_fsize - 1;
  if (self->m_fpos < 0) self->m_fpos = 0;
...
static PyMethodDef ChinaL1Source_methods[] = {
    {"load", (PyCFunction)ChinaL1Source_load, METH_VARARGS,
     "Load ChinaL1 Message"},
    {"at", (PyCFunction)ChinaL1Source_at, METH_VARARGS,
     "Get index ChinaL1 Message"},
    {"next", (PyCFunction)ChinaL1Source_next, METH_NOARGS,
     "Get next ChinaL1 Message"},
    {NULL, NULL, 0, NULL} /* Sentinel */
};

PyTypeObject ChinaL1Source_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "lmice.ChinaL1Source", /* tp_name */
    ...
    0,                                                    /* tp_setattr */
    0,                                                    /* tp_compare */
    (reprfunc)ChinaL1Source_repr,                         /* tp_repr */
    0,                                                    /* tp_as_number */
    0,                                                    /* tp_as_sequence */
    0,                                                    /* tp_as_mapping */
    0,                                                    /* tp_hash */
...
void init_chinal1source_object(PyObject *m) {
  if (PyType_Ready(&ChinaL1Source_Type) < 0) return;
