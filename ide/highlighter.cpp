...
Highlighter::Highlighter(QTextDocument *parent) : QSyntaxHighlighter(parent) {
  ...
  rule.pattern = QRegularExpression(
      ...
      "load\\b"
      "(?=\\()");
  classFormat.setToolTip("load guava file");
  rule.format = classFormat;
  qDebug() << rule.format.toolTip();
...
void Highlighter::highlightBlock(const QString &text) {
  ...
  //    //! Package load
  //    QRegularExpression loadExp =
  //    QRegularExpression("^\\s*load_guava2_package\\("); QRegularExpression
  //    loadEndExp = QRegularExpression("\\)"); int loadIndex =
  //    text.indexOf(loadExp); if(loadIndex >= 0)
  //    {
  ...
  QString triplequote = "\"\"\"";
  startIndex = 0;
  if (!(prevstate & TripleDoubleQuote)) startIndex = text.indexOf(triplequote);
  while (startIndex >= 0) {
    int endIndex = 0;
  ...
  triplequote = "'''";
  startIndex = 0;
  if (!(prevstate & TripleQuote)) startIndex = text.indexOf(triplequote);
  while (startIndex >= 0) {
    int endIndex = 0;
